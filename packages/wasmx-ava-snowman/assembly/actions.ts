...
export function setupNode(
    ...
    fsm.setContextValue(CURRENT_NODE_ID, data.node_index.toString());
...
function startBlockFinalizationInternal(entryobj: LogEntryAggregate, retry: boolean): boolean {
    ...
    for (let i = 0; i < evs.length; i++) {
        const ev = evs[i];
        if (ev.type == wasmevs.EventTypeRegisterRole) {
            for (let j = 0; j < ev.attributes.length; j++) {
                if (ev.attributes[j].key == wasmevs.AttributeKeyRole) {
                    roleConsensus = ev.attributes[j].value == "consensus"
                }
                if (ev.attributes[j].key == wasmevs.AttributeKeyContractAddress) {
                    newContract = ev.attributes[j].value;
                }
                if (ev.attributes[j].key == wasmevs.AttributeKeyRoleLabel) {
